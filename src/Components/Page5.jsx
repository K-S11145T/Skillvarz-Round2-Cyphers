...
const Page5 = () => {
  const products = [
    ...
    },
    {
      ...
      price: "$1999",
      model: "Major V.",
      details: "Major V. delivers exceptional sound quality with advanced noise cancellation and a sleek, modern design. Perfect for music lovers on the go.",
      ratings: "4.7 Stars",
    ...
    },
    {
      ...
      price: "$2499",
      model: "Motif 2",
      details: "Motif 2 offers a perfect balance of style and performance with its compact design and powerful sound output. Ideal for any setting.",
      ratings: "4.8 Stars",
        ...
        <div className='BottomContainer w-[70%] h-[20%] absolute right-0 bottom-0 flex'>
          ...
          <div className='ProductNames w-[50%] h-full lg:px-6 xl:pb-10'>
            {products.map((product, index) => (
              <div
                key={index}
                className='justify-between flex lg:h-10 2xl:h-18 items-center border-b-2 border-[#FFFFFF] cursor-pointer'
                onClick={() => handleProductClick(product)}
              >
                <h1 className='lg:text-xl 2xl:text-3xl font-[Montserrat]'>{product.name}</h1>
                <h3 className='lg:text-xl 2xl:text-3xl font-[Silk-Serif-Light-Italic]'>{`0${index + 1}`}</h3>
              </div>
            ))}
          </div>
        </div>
